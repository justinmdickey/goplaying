...
### Overview
Releases are **fully automated**. Pushing a `v*` tag triggers `.github/workflows/build.yml`, which builds all platforms and updates every distribution channel with **zero manual steps**:

- GitHub Release (binaries + checksums) — GoReleaser
- Homebrew tap `justinmdickey/homebrew-tap` — GoReleaser
- AUR `goplaying-bin` (multi-arch x86_64 + aarch64) — GoReleaser `aurs:` block
- AUR `goplaying-git` (VCS package, pkgver bump signal) — `update-aur-git` job

Do not update AUR packages by hand anymore. See "Automation internals" below.

### Version Numbering
- **Format**: `v0.MINOR.PATCH` (0-based versioning during development)
- **Examples**: v0.3.6, v0.3.7, v0.3.8
- **When to bump**:
...
git log --oneline -5   # confirm the changes to release are merged
git status             # clean tree
```

#### 2. Tag and push
The tag must point at a `main` commit — the workflow that runs is the one committed at the tagged ref.
```bash
git tag -a v0.3.X -m "Release v0.3.X - Brief description
...
#### 3. Watch the run
```bash
gh run watch $(gh run list --workflow build.yml --limit 1 --json databaseId -q '.[0].databaseId') --exit-status
```
Two-stage pipeline: `build-swift-helper` (macOS) + `release` (GoReleaser) run first, then `update-aur-git` runs after `release` succeeds. Expect all jobs green in ~2-3 min.

#### 4. Verify (optional — all automated)
```bash
gh release view v0.3.X
# AUR pkgver landed:
git clone ssh://aur@aur.archlinux.org/goplaying-git.git /tmp/g && grep ^pkgver /tmp/g/PKGBUILD
git clone ssh://aur@aur.archlinux.org/goplaying-bin.git /tmp/b && grep ^pkgver /tmp/b/PKGBUILD
```

### Automation internals

- **Trigger**: `.github/workflows/build.yml` on `push` tags `v*`.
- **Secrets** (repo → Settings → Secrets):
  - `HOMEBREW_TAP_GITHUB_TOKEN` — push to the Homebrew tap.
  - `AUR_KEY` — private SSH key whose public half is on the AUR account. Used by both the GoReleaser `aurs:` block and the `update-aur-git` job.
- **`goplaying-bin`**: generated by GoReleaser's `aurs:` block in `.goreleaser.yml` from the `linux-targz` archive. **GoReleaser owns this PKGBUILD** — never hand-edit it; changes come from the `aurs:` config.
- **`goplaying-git`**: bumped by the `update-aur-git` job (arch container). Computes `pkgver=r<count>.<short>`, seds PKGBUILD, regenerates `.SRCINFO` with `makepkg --printsrcinfo` as an unprivileged user, and pushes. `--printsrcinfo` uses the static pkgver (does not run the VCS `pkgver()`), so the sed value is authoritative.
- **Local AUR clones** (`~/Dev/aur/goplaying-{bin,git}`) go stale after each release since CI commits ahead. `git pull` before touching them.

### Distribution Channels

| Channel | Package | Updated by | Install |
| --- | --- | --- | --- |
| GitHub Releases | binaries + checksums | GoReleaser | download from Releases |
| Homebrew | `justinmdickey/tap/goplaying` (macOS) | GoReleaser | `brew install justinmdickey/tap/goplaying` |
| AUR | `goplaying-bin` (x86_64 + aarch64) | GoReleaser `aurs:` | `yay -S goplaying-bin` |
| AUR | `goplaying-git` (VCS, tracks HEAD) | `update-aur-git` job | `yay -S goplaying-git` |

### Release Checklist

Before tagging:
- [ ] All changes merged to main
- [ ] Code formatted (`make fmt`), tests passing (`make test`)
- [ ] README updated if needed
- [ ] Version number decided (MINOR vs PATCH)

After tagging (all automated — just confirm):
- [ ] `build.yml` run is green (all jobs, including `update-aur-git`)
- [ ] GitHub Release created with assets
- [ ] Homebrew formula shows new version
- [ ] Both AUR packages show new version

### Troubleshooting

**GitHub Actions fails**:
- Check logs: `gh run view <run-id> --log-failed`
- Common issues: GoReleaser config syntax, missing/expired `AUR_KEY` or `HOMEBREW_TAP_GITHUB_TOKEN`.

**Homebrew not updating**:
- Check `.goreleaser.yml` `brews:` block and the `justinmdickey/homebrew-tap` repo.

**AUR not updating**:
- Check the relevant job in the `build.yml` run.
- `update-aur-git` runs git as root in a container — if you see "dubious ownership", ensure the `safe.directory` config is present in the "Compute pkgver" step.
- AUR API cache refreshes in 1-2 minutes; verify the push in the AUR git repo directly.

**Re-running a release** (e.g. after fixing a workflow bug): delete and re-push the tag at the fixed `main` commit. GoReleaser `mode: replace` re-publishes the GitHub Release identically.
```bash
git push origin :refs/tags/v0.3.X
git checkout main && git pull
git tag -f v0.3.X -m "Release v0.3.X"
git push origin v0.3.X
```
